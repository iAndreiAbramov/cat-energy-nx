      ...
      {products.length > 0 ? (
        <>
          <ul className={styles['area-list']}>
            {products.map(({ name, basicUnit, weightGram, priceRub }) => (
              <li
                key={name}
                className={cn(
          ...
          <GiftCard
            className={cn(styles['area-gift'], 'mt-9 md:mt-[68px] xl:mt-0')}
          />
        </>
      ) : (
        <p className="font-arial text-sm md:text-base">Здесь пока ничего нет</p>
      )}
    </section>
  );
};
